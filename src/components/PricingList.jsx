      ...
      {pricing.map((item) => (
        <div key={item.id} className="w-[19rem] max-lg:w-full h-full px-6 bg-n-8 border border-n-6 rounded-[2rem] lg:w-auto even:py-14 odd:py-8 odd:my-4 [&>h4]:first:text-color-2 [&>h4]:even:text-color-1 [&>h4]:last:text-color-3">
            ...
            <div className="flex items-center h-[5.5rem] mb-6">
                {item.price && (
                  <> {/* we need to wrap it in a fragment or <> (can also use <div>) because in jsx you can't return multiple elements so need to put it inside a parent element which is the fragment*/}
                    <div className="h3">$</div>
                    <div className="text-[5.5rem] leading-none font-bold">{item.price}</div>
                  </>
                )}
            </div>
            <Button className="w-full mb-6" href={item.price ? '/pricing' : 'mailto:brainwave@gmail.com'} white={!!item.price}>
                {item.price ? "Get started" : "Contact us"}
            </Button>
            <ul>
                {item.features.map((feature, index) => (
                    <li key={index} className="flex items-start py-5 border-t border-n-6">
                        <img className="mr-4" src={check} width={24} height={24} alt="check"/>
